;(function () {
	...
	$('.shopping-cart').click(function(event) {
		...
		var cartElement = $('.menu-2 #cart');

		if ( cartElement.hasClass('hidden') ) {
			cartElement.removeClass('hidden');
		} else {
			cartElement.addClass('hidden');
		}
	});

	$('body').click(function(event){
		if(event.target.id === "cart" || $(event.target).closest('#cart').length !== 0)
			return;

		$('.menu-2 #cart').addClass('hidden')
	});
